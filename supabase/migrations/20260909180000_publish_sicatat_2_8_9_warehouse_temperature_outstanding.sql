-- Android publishing is intentionally limited to arm64-v8a at the product owner's request.
do $$
...
begin
  ...
  if not exists (select 1 from storage.objects where bucket_id = 'app-releases' and name = 'arm64-v8a/app-arm64-v8a-2.8.9-release.apk') then raise exception 'Upload the arm64-v8a 2.8.9 APK before activating this release.'; end if;
  ...
  insert into public.app_release (platform,abi,version_name,version_code,apk_path,release_notes,is_active,created_by) values
    ('android','arm64-v8a','2.8.9',12289,'arm64-v8a/app-arm64-v8a-2.8.9-release.apk',E'• Gudang menampilkan tanggal pembaruan dari spreadsheet.\n• Tampilan menu Suhu desktop lebih ringkas.\n• Outstanding PM/CM mengikuti crew dan lokasi CPP/PORT.',true,release_creator)
