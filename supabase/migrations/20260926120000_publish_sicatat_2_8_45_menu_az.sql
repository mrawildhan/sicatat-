...
begin
  ...
  if not exists (
    ...
    where bucket_id = 'app-releases'
      and name = 'arm64-v8a/app-arm64-v8a-2.8.45-release.apk'
  ) then
    raise exception 'Upload APK arm64-v8a SICATAT 2.8.45 sebelum mengaktifkan rilis.';
  end if;
  ...
  insert into public.app_release (
    ...
    '2.8.45',
    14325,
    'arm64-v8a/app-arm64-v8a-2.8.45-release.apk',
    E'• Semua menu kini berurutan abjad A–Z: Suhu, Gudang, aktivitas lembar suhu, dan Profil.
• Termasuk seluruh isi 2.8.44: Tugas saya, tindak lanjut suhu kritis, kepatuhan pengisian, Laporan Bulanan, riwayat aset, dan ekspor Excel.',
    true,
    release_creator
