-- From this release onward, Android publishing is intentionally limited to
-- arm64-v8a at the product owner's request.
do $$
...
begin
  ...
  if not exists (select 1 from storage.objects where bucket_id = 'app-releases' and name = 'arm64-v8a/app-arm64-v8a-2.8.8-release.apk') then raise exception 'Upload the arm64-v8a 2.8.8 APK before activating this release.'; end if;
  ...
  insert into public.app_release (platform,abi,version_name,version_code,apk_path,release_notes,is_active,created_by) values
    ('android','arm64-v8a','2.8.8',12288,'arm64-v8a/app-arm64-v8a-2.8.8-release.apk',E'• Status sinkronisasi khusus Suhu tidak lagi tampil di Beranda.\n• Beranda kini kembali fokus pada menu utama.',true,release_creator)
