...
begin
  ...
  if not exists (select 1 from storage.objects where bucket_id = 'app-releases' and name = 'arm64-v8a/app-arm64-v8a-2.8.10-release.apk') then raise exception 'Upload the arm64-v8a 2.8.10 APK before activating this release.'; end if;
  ...
  insert into public.app_release (platform,abi,version_name,version_code,apk_path,release_notes,is_active,created_by) values
    ('android','arm64-v8a','2.8.10',12290,'arm64-v8a/app-arm64-v8a-2.8.10-release.apk',E'• Outstanding PM menampilkan data CPP dan PORT dari spreadsheet.\n• Kartu Crew A/B/C per CPP/PORT dapat ditekan untuk membuka daftar work order outstanding.\n• CM tetap menunggu spreadsheet progres yang diperbarui.',true,release_creator)
