-- Store optional source documents separately from mandatory completion proof.

alter table public.operational_reminder
  add column if not exists document_number text;

alter table public.operational_reminder_evidence
  add column if not exists attachment_type text not null default 'completion_proof';

alter table public.operational_reminder_evidence
  drop constraint if exists operational_reminder_evidence_attachment_type_check,
  add constraint operational_reminder_evidence_attachment_type_check
    check (attachment_type in ('supporting_document', 'completion_proof'));

create or replace function public.attach_operational_reminder_documents(
  p_reminder_id uuid,
  p_documents jsonb
)
returns void
...
begin
  ...
  if jsonb_typeof(p_documents) <> 'array'
     or jsonb_array_length(p_documents) not between 1 and 5 then
    raise exception 'Attach from one to five supporting documents.' using errcode = '22023';
  end if;
  ...
  if reminder.status <> 'open'
     or not public.can_manage_sicatat_reminder_site(reminder.site_id) then
  ...
  for document in select value from jsonb_array_elements(p_documents) loop
    document_path := trim(coalesce(document ->> 'storage_path', ''));
    document_name := trim(coalesce(document ->> 'file_name', ''));
    document_mime := trim(coalesce(document ->> 'mime_type', ''));
    document_size := nullif(document ->> 'size_bytes', '')::integer;
    if document_path not like p_reminder_id::text || '/%'
       or document_name !~* '\.(pdf|jpe?g|png)$'
       or document_mime not in ('application/pdf', 'image/jpeg', 'image/png')
       or document_size not between 1 and 10485760
       or not exists (
         select 1 from storage.objects
         where bucket_id = 'reminder-evidence'
           and name = document_path
       ) then
      raise exception 'One or more supporting documents are invalid.' using errcode = '22023';
    end if;
  end loop;
  ...
  for document in select value from jsonb_array_elements(p_documents) loop
    insert into public.operational_reminder_evidence (
      ...
      trim(document ->> 'storage_path'),
      trim(document ->> 'mime_type'),
  ...
  insert into public.operational_reminder_activity (
    ...
    'Supporting documents attached when the reminder was created.',
    jsonb_build_object('file_count', jsonb_array_length(p_documents)),
    actor_id
  );
end;
$$;

grant execute on function public.attach_operational_reminder_documents(uuid, jsonb)
  to authenticated;
...
begin
  ...
  if reminder.recurrence_months is not null then
    ...
    insert into public.operational_reminder (
      ...
      document_number, description, priority, assigned_to, location,
      reminder_offsets_days, reminder_schedule, custom_reminder_days,
      ...
      reminder.description, reminder.priority, reminder.assigned_to,
      reminder.location, reminder.reminder_offsets_days,
