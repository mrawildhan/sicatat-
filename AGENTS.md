...
- **Tindak lanjut suhu kritis** (Suhu → Alat bantu, reviewer): `temperature_alert` punya `followup_status` (open/in_progress/closed), tindakan, No. WO, foto, `followup_by/at`, `closed_at`. Semua disimpan hanya lewat RPC `temperature_alert_follow_up` (cek `can_review_daily_check`; tidak bisa ditutup tanpa tindakan) yang juga menulis `audit_log` (`entity_type='temperature_alert'`, dengan policy baca untuk reviewer). Foto dikompres ~300 KB ke bucket privat `temperature-alert-photos` (`<alert id>/<ms>.jpg|png`, maks. 1 MB). Layar `/temperature-alerts` (`daily_checks/presentation/temperature_alerts_screen.dart`) bisa diekspor ke PDF/Excel.
- **Kepatuhan pengisian** (`/compliance`, reviewer): kalender bulanan 3 lembar × 2 shift. Crew diambil dari `roster_anchor`; tepat waktu berarti dikirim ≤ 2 jam setelah shift berakhir (Pagi 19.00, Malam 07.00). Ada persentase per crew dan per lembar, daftar lembar yang belum disetujui, serta ekspor PDF/Excel. Foreman hanya melihat crew-nya. Logikanya di `daily_checks/compliance.dart` (dites).
- **Riwayat aset** (`/asset-history/:ref`, dari Referensi Alat → detail → "Riwayat aset", dan dari detail PR): PM/CM terbuka, PR dengan referensi alat itu, Outstanding PO dan penerimaan dari PO PR tersebut, pengambilan dengan nomor WO unit, serta suhu kritis 12 bulan. Suhu dipetakan: AFB01 = breaker + hydraulic, ACR01 = sizer, AGV1–4 = RV01–04. Bisa diekspor ke PDF.
- **Tugas saya** di Beranda (`dashboard/presentation/my_tasks_card.dart`): PM crew sendiri yang jatuh tempo dalam 7 hari, barang pesanan (nama pengguna = requestor) yang belum atau baru datang, suhu kritis yang belum ditutup, lembar yang menunggu persetujuan, dan alat yang terlambat kembali (> `warehouseLoanOverdueDays`). Sumber yang tidak boleh dibaca oleh peran pengguna dilewati. Barang dipesan menerima `?requestor=`.
- **Data PR**: detail PR menampilkan alur PR → PO → menunggu barang → datang, beserta jumlah harinya (`pr_tracking.dart`). Sebelum mencari, tampil "Umur PR terbuka" (rilis 12 bulan terakhir; hijau sudah ada PO, oranye belum) yang bisa diketuk untuk melihat daftar. Hasil pencarian bisa diekspor ke Excel. Catatan: `no_po` di PR.xlsx sering berisi teks (WAS APPROVED dsb.), dan PO PR lama belum cocok dengan penerimaan, sehingga alur sering berhenti di "PO".
- **Tren backlog PM & CM**: tabel `maintenance_backlog_snapshot` diisi `maintenance_record_backlog()` oleh cron `sicatat-maintenance-backlog-daily` (23.30 WITA). Titik pertama tercatat 2026-09-26 (PM 149, CM 59). Grafik dan perbandingan 7/30 hari ada di PM & CM Tertunda (`maintenance_backlog_trend.dart`).
- **Anggaran**: grafik per bulan (anggaran vs aktual, merah bila lebih), kumulatif + proyeksi (rata-rata aktual × 12), dan elemen biaya di atas anggaran sampai bulan aktual terakhir (`budget_charts.dart`).
- **Laporan Bulanan** (Operasional, reviewer; `/monthly-report`): satu PDF berisi kepatuhan, suhu kritis dan tindak lanjutnya, PM & CM, anggaran, PR, serta gudang (`reports/monthly_report.dart`).
- **Ekspor Excel** (`core/export/xlsx_export.dart`, xlsx ditulis manual, font Aptos, header hijau, filter, tanggal Excel asli) untuk Barang dipesan, Cari barang, Data PR, Peminjaman alat, suhu kritis, dan kepatuhan. Web mengunduh, Android memakai share sheet.
- **Pinjaman alat terlambat**: chip "Terlambat > 3 hari (N)" di Peminjaman Alat untuk tab SICATAT dan LIST ORDER (LIST ORDER diurutkan dari yang tertua), plus ekspor Excel alat yang belum kembali.
- Tampilan PDF bersama ada di `core/pdf/report_pdf.dart`. Query PostgREST sudah dicek dengan service_role (GET saja), dan RPC dicek sebagai admin lewat transaksi yang di-rollback. Layar belum dicek visual di browser karena login butuh PIN pemilik. 97 tes lulus.
...
- **Stok: sumber yang tanggalnya paling baru menang** (pemilik, 2026-09-25: "yang pasti jika saya cari barang itu real"). Laporan Ellipse (`source_updated_on` = RUN DATE) dan sheet SCALLSITE/Tanya WHS (TANGGAL UPDATE) dibandingkan per barang; kalau tanggalnya sama, Ellipse yang dipakai. Detail barang (part no, bin, kelas) tetap dari Ellipse. RPC `warehouse_apply_sheet_stock` (khusus service_role, migrasi `20260925130000`) memperbarui SOH, dan `importInventory` tidak menimpa stok sheet yang lebih baru. Kolom `warehouse_stock.stock_from` (`ellipse`/`sheet`) ditampilkan di detail Cari barang sebagai "Stok per <tanggal> · <sumber>". Barang sheet tanpa deskripsi masuk sebagai "SC <kode>". "Tanya WHS" di aplikasi teman membaca sheet SCALLSITE yang sama (Apps Script), jadi tidak perlu disambung terpisah. Setelah ini semua 7.722 barang AMWH/KMWH/MAIN memakai stok sheet 25/9. Perbandingan 2026-09-25: SCALLSITE (25/9) vs SICATAT (Ellipse 24/9) sama untuk AMWH, beda 16 barang di KMWH dan 27 di MAIN (kebanyakan SOH kosong/1 di sheet). "Tanya whs" tidak tersambung.
